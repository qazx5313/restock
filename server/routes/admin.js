...
router.get('/users', async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE users SET status='expired' WHERE status='active' AND expires_at IS NOT NULL AND expires_at < NOW()`);
    const countResult = await pool.query(`SELECT COUNT(*) FROM users ${whereClause}`, params);
    params.push(limit, offset);
    const result = await pool.query(
      `SELECT id, nickname, username, role, status, expires_at, created_at FROM users ${whereClause} ORDER BY ${sortCol} ${sortOrder} LIMIT $${params.length - 1} OFFSET $${params.length}`,
      params
    );
...
router.post('/users/:id/activate', async (req, res) => {
  try {
    const { id } = req.params;
    const { days } = req.body;
    if (!days || days < 1) return res.status(400).json({ error: '請提供有效天數' });
    const expiresAt = new Date();
    expiresAt.setDate(expiresAt.getDate() + parseInt(days));
    const result = await pool.query(
      `UPDATE users SET status='active', expires_at=$1, updated_at=NOW() WHERE id=$2 AND role != 'admin' RETURNING id, nickname, username, status, expires_at`,
      [expiresAt, id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: '使用者不存在' });
    await pool.query('INSERT INTO admin_logs (admin_id, action, target_user_id, detail) VALUES ($1,$2,$3,$4)', [req.user.id, 'activate', id, `開通${days}天`]);
...
router.post('/users/:id/revoke', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(`UPDATE users SET status='pending', expires_at=NULL, updated_at=NOW() WHERE id=$1 AND role != 'admin' RETURNING id, nickname, username, status`, [id]);
    if (result.rows.length === 0) return res.status(404).json({ error: '使用者不存在' });
    await pool.query('INSERT INTO admin_logs (admin_id, action, target_user_id, detail) VALUES ($1,$2,$3,$4)', [req.user.id, 'revoke', id, '取消權限']);
...
router.post('/users/:id/disable', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(`UPDATE users SET status='disabled', updated_at=NOW() WHERE id=$1 AND role != 'admin' RETURNING id, nickname, username, status`, [id]);
    if (result.rows.length === 0) return res.status(404).json({ error: '使用者不存在' });
    await pool.query('INSERT INTO admin_logs (admin_id, action, target_user_id, detail) VALUES ($1,$2,$3,$4)', [req.user.id, 'disable', id, '停用帳號']);
...
router.put('/params', async (req, res) => {
  try {
    ...
    if (!params || typeof params !== 'object') return res.status(400).json({ error: '參數格式錯誤' });
    for (const [key, value] of Object.entries(params)) {
      await pool.query('UPDATE tech_params SET param_value=$1, updated_at=NOW() WHERE param_key=$2', [String(value), key]);
    }
...
router.post('/reports/ai-generate', async (req, res) => {
  try {
    ...
    const dbResult = await pool.query(
      `INSERT INTO reports (title, report_date, market_trend, summary, content, status, created_by) VALUES ($1,$2,$3,$4,$5,'draft',$6) RETURNING *`,
...
router.get('/reports', async (req, res) => {
  try {
    const result = await pool.query(`SELECT r.*, u.nickname as author_name FROM reports r LEFT JOIN users u ON r.created_by = u.id ORDER BY r.report_date DESC, r.created_at DESC`);
...
router.post('/reports', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO reports (title, report_date, market_trend, summary, content, status, created_by) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
...
router.put('/reports/:id', async (req, res) => {
  try {
    ...
    const { title, report_date, market_trend, summary, content, status } = req.body;
    const result = await pool.query(
      `UPDATE reports SET title=$1, report_date=$2, market_trend=$3, summary=$4, content=$5, status=$6, updated_at=NOW() WHERE id=$7 RETURNING *`,
